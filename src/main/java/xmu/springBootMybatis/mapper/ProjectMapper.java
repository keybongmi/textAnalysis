...
public interface ProjectMapper {
	...
	@Select("select * from project where user_id = #{user_id} and finish_or_not=#{finish_or_not} ORDER BY id ASC")
	...
	@Update("update project set statistical_analysis=#{chooseNode},classification_algorithm=#{classification_algorithm},"
			+ "clustering_algorithm=#{clustering_algorithm},Kmeans_value=#{Kmeans_value}, SVM_value=#{SVM_value},"
			+ "linear_regression_value =#{linear_regression_value},Kmeans_knowledge_value=#{Kmeans_knowledge_value} "
			+ "where id=#{projectId}")
	public int updateProjectAnotherMethod(@Param("chooseNode") boolean chooseNode,
			...
			@Param("linear_regression_value")String linear_regression_value,
			@Param("Kmeans_knowledge_value")String Kmeans_knowledge_value);
